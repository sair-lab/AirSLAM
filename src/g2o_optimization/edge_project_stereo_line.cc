...
void EdgeStereoSE3ProjectLine::computeError() {
  ...
  error(2) = (obs(5) * line_2d_norm_right(0) + obs(6) * line_2d_norm_right(1) + line_2d_norm_right(2)) / line_2d_norm_right;
  error(3) = (obs(7) * line_2d_norm_right(0) + obs(8) * line_2d_norm_right(1) + line_2d_norm_right(2)) / line_2d_norm_right;
  _error = error;
}
